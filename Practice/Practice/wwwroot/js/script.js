...
$(document).ready(function () {
    ...
    //DeleteProductImage(AdminProductEdit)
    ...
    $(document).on("click", ".delete-image-btn", function () {
        ...
        //eger keyler metodun qebul etdiyi parametrin(yeni objectin) icindekilerle(propertylerle) adlari eyni
        //olarsa 1 defe yazsaq bes edir. Yeni imageId = imageId yazmaqa ehtiyac yoxdur. AMMA propertyleri
        //PascalCase yaziriq Javascriptde ise camelCase yazmaliyiq
    ...
    $(document).on("click", ".set-main-image-btn", function () {
        ...
        let imageId = parseInt($(this).attr("data-image-id"))
        let productId = parseInt($(this).attr("data-product-id"))
        let parent = $(this).parent().children().first()
        console.log(parent)

        $.ajax({
            url: `/admin/product/SetMainImage?id=${imageId}&productId=${productId}`,
            type: "POST",
            success: function (response) {
                $(".main-product-image").removeClass("main-product-image")
                parent.addClass("main-product-image");
            },
        });
